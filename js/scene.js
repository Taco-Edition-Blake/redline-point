...
const MAP_SCALE = 1.7; // the source map's real footprint is only ~25x16.6m — scale it up for a roomier 1v1 arena
const SPAWN_A = new THREE.Vector3(0, 0, -6.5 * MAP_SCALE);
const SPAWN_B = new THREE.Vector3(0, 0, 6.5 * MAP_SCALE);
...
const MAX_STEP_UP = 0.6; // max height a player can "step up" in one frame (stairs/curbs), not "teleport onto a roof"

function getGroundHeightWide(x, z, fallback){
  // used only at spawn/reset time, when we have no reliable "current height" to search near
...
function getGroundHeight(x, z, currentFeetY){
  // used every frame during movement: searches a narrow band around the current
  // feet height so a downward ray can't jump onto a rooftop just because it's
  // the first thing directly overhead — walking near a tall wall no longer
  // launches you onto it.
  if(!mapRoot) return currentFeetY;
  _groundOrigin.set(x, currentFeetY + MAX_STEP_UP + 0.05, z);
  _groundRay.set(_groundOrigin, _groundDir);
  _groundRay.far = MAX_STEP_UP + 3.0;
  const hits = _groundRay.intersectObject(mapRoot, true);
  return hits.length ? hits[0].point.y : currentFeetY;
}

/**
 * Shared vertical physics for anything standing on the map (player or bot).
 * `state` is a plain object with { feetY, velocityY, grounded } that gets
 * mutated in place. Small height changes (stairs/curbs) snap immediately;
 * anything bigger — walking off a ledge, jumping — falls under real gravity
 * instead of teleporting straight down.
 */
function applyGroundPhysics(state, x, z, dt, jumpRequested){
  const groundHere = getGroundHeight(x, z, state.feetY);
  ...
  const closeToGround = state.grounded && Math.abs(groundHere - state.feetY) <= MAX_STEP_UP;
  if(closeToGround){
    state.feetY = groundHere;
    state.velocityY = 0;
    state.grounded = true;
  } else {
    const proposed = state.feetY + state.velocityY*dt;
    if(proposed <= groundHere){
      state.feetY = groundHere;
      state.velocityY = 0;
      state.grounded = true;
    } else {
      state.feetY = proposed;
      state.grounded = false;
    }
  }
  return state.feetY;
...
async function loadAllAssets(onProgress){
  ...
  for(let i=0;i<keys.length;i++){
    ...
    if(key==='map'){
      mapRoot = gltf.scene;
      mapRoot.scale.setScalar(MAP_SCALE);
      scene.add(mapRoot);
