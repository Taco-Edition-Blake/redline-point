...
let velocityY = 0, grounded = true, playerHopY = 0;
...
function resetPlayer(pos){
  ...
  camera.rotation.x = 0;
  velocityY = 0; playerHopY = 0; grounded = true; isAiming = false;
  yawObject.position.y = getGroundHeight(pos.x, pos.z, 0) + EYE_OFFSET;
...
function updateMovement(dt){
  ...
  const feetY = yawObject.position.y - EYE_OFFSET;
  resolveCollision(next, feetY, 0.5);

  if(keys.Space && grounded){ velocityY = JUMP_SPEED; grounded = false; }
  velocityY += GRAVITY*dt;
  playerHopY = Math.max(0, playerHopY + velocityY*dt);
  if(playerHopY <= 0){ velocityY = 0; grounded = true; }

  const groundY = getGroundHeight(next.x, next.z, feetY);
  yawObject.position.x = next.x;
  yawObject.position.z = next.z;
  yawObject.position.y = groundY + EYE_OFFSET + playerHopY;
}
...
function updateBot(dt){
  ...
  const botFeetY = getGroundHeight(opponent.pos.x, opponent.pos.z, 0) + opponent.hopY;
  resolveCollision(next, botFeetY, 0.5);
