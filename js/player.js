...
const playerPhysics = { feetY: 0, velocityY: 0, grounded: true };
...
function resetPlayer(pos){
  ...
  camera.rotation.x = 0;
  isAiming = false;
  playerPhysics.feetY = getGroundHeightWide(pos.x, pos.z, 0);
  playerPhysics.velocityY = 0;
  playerPhysics.grounded = true;
  yawObject.position.y = playerPhysics.feetY + EYE_OFFSET;
...
document.addEventListener('pointerlockchange', () => {
  const nowLocked = document.pointerLockElement === UI.gameCanvas;
  if(!nowLocked && pointerLocked && gameActive && !roundFrozen){
    paused = true;
  ...
  UI.clickToPlay.classList.toggle('hidden', pointerLocked || !gameActive || paused);
});
UI.gameCanvas.addEventListener('click', () => { if(gameActive && !pointerLocked && !paused) UI.gameCanvas.requestPointerLock(); });
UI.clickToPlay.addEventListener('click', () => { if(gameActive && !paused) UI.gameCanvas.requestPointerLock(); });
UI.gameCanvas.addEventListener('contextmenu', e => e.preventDefault());
UI.resumeBtn.addEventListener('click', () => { UI.gameCanvas.requestPointerLock(); });
UI.pauseHomeBtn.addEventListener('click', () => quitToMenu());
...
function startRound(){
  ...
  resetPlayer(SPAWN_A);
  if(opponent){
    opponent.health = 100; opponent.shield = 100; opponent.physics.velocityY = 0; opponent.physics.grounded = true;
    opponent.setPose(SPAWN_B, Math.PI); opponent.pos.copy(SPAWN_B);
    opponent.physics.feetY = getGroundHeightWide(SPAWN_B.x, SPAWN_B.z, 0);
  }
...
function updateMovement(dt){
  ...
  resolveCollision(next, playerPhysics.feetY, 0.5);

  applyGroundPhysics(playerPhysics, next.x, next.z, dt, !!keys.Space);
...
/* ============================================================
   BOT AI (quickplay)
   ============================================================ */
let botFireCooldown = 0, botStrafeDir = 1, botStrafeT = 0, botSprinting = true, botJumpTimer = 1.5;
...
function updateBot(dt){
  ...
  resolveCollision(next, opponent.physics.feetY, 0.5);
  opponent.setPose(next, Math.atan2(-dir.x, -dir.z));

  botJumpTimer -= dt;
  if(botJumpTimer <= 0){
    botJumpTimer = 1.2 + Math.random()*2.2;
    if(opponent.physics.grounded && Math.random() < 0.55) opponent.wantsJump = true;
  }
